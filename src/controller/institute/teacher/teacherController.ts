...
const createTeacher = async(req:IExtendedRequest, res:Response)=>{
  ...
  const insertedData = await sequelize.query(`INSERT INTO teacher_${instituteNumber}(teacherName, teacherEmail, teacherPhoneNumber, teacherExpertise, joinedData, salary, teacherPhoto, teacherPassword) VALUES(?,?,?,?,?,?,?,?)`, {
